#File name: Differential Privacy and Quantile Regression Sensitivity of Estimator (Non-Normal Case)
...
SensEst <- function(ss=1000, sigma=0.25, niter=10000, h=100) {
  ...
  for (i in 1:niter) {
    ...
    #print(dim(x))
    y=x[,1]+x[,2]-x[,3]+2*x[,4]-x[,5]+(sigma*rt(nrow(x), 1, 0))
    #print(length(y))
    rqmodel <- rq(y~x, 0.5)
    result = summary(rqmodel)$coefficients[2:6,1]
    y0=h+h-h+2*h-h+(sigma*rt(1, 1, 0))
    #print(length(y0))
    y1=c(y,y0)
    x0=matrix(h, 1, 5)
    x1=rbind(x, x0)
...
res1k25test = SensEst(ss=1000, sigma=0.25, niter=100, h=10)
